...
export default function NomencladorPage() {
  ...
  const [editing, setEditing] = useState<Nomenclador | null>(null);
  const [form, setForm] = useState<NomencladorForm>(EMPTY_FORM);
  const [saving, setSaving] = useState(false);
  const [formError, setFormError] = useState<string | null>(null);

  const [deleteTarget, setDeleteTarget] = useState<Nomenclador | null>(null);
  ...
  const filteredNomencladores = nomencladores.filter((n) => {
    ...
    const label = nomencladorLabel(n).toLowerCase();
    const matchesSearch =
      !term ||
      label.includes(term) ||
      n.discipline.toLowerCase().includes(term) ||
      n.subdiscipline.toLowerCase().includes(term) ||
      n.specialty.toLowerCase().includes(term);
    const matchesStatus =
      statusFilter === "all" ||
  ...
  const handleSubmit = async () => {
    setFormError(null);
    if (!form.discipline.trim() || !form.subdiscipline.trim() || !form.specialty.trim()) {
      setFormError(
        "La disciplina, la subdisciplina y la especialidad son obligatorias.",
      );
      return;
    }
    setSaving(true);
    try {
      ...
      setFormError(
        e instanceof Error ? e.message : "No se pudo guardar el nomenclador.",
      );
    } finally {
      setSaving(false);
    }
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await nomencladorApi.remove(deleteTarget.id);
      setNomencladores((prev) => prev.filter((n) => n.id !== deleteTarget.id));
      setDeleteTarget(null);
    } catch (e) {
      setError(
        e instanceof Error ? e.message : "No se pudo eliminar el nomenclador.",
      );
      setDeleteTarget(null);
    } finally {
      setDeleting(false);
  ...
  return (
    <div>
      <PageMeta title="Academix" description="Gestión del nomenclador" />
      <PageBreadcrumb pageTitle="Nomenclador" />
        ...
        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
          <div className="flex flex-col gap-4 border-b border-gray-200 px-5 py-4 dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between">
            <h2 className="font-semibold text-gray-900 dark:text-white">
              Nomenclador de disciplina, subdisciplina y especialidad
            </h2>
            <Button
              onClick={openCreate}
              size="sm"
          ...
          <div className="flex flex-col gap-3 border-b border-gray-200 px-5 py-4 dark:border-gray-800 sm:flex-row sm:items-center">
            <div className="relative flex-1">
              <input
                ...
                placeholder="Buscar por disciplina, subdisciplina o especialidad…"
          ...
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <div className="px-5 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
                No se encontraron nomencladores con los criterios de búsqueda.
              </div>
            ) : (
              <table className="w-full min-w-[760px] text-left text-sm">
                <thead className="bg-gray-50 text-xs uppercase text-gray-500 dark:bg-white/[0.02] dark:text-gray-400">
                  <tr>
                    ...
                    <th className="px-5 py-3">Subdisciplina</th>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            ...
                            aria-label={`Eliminar ${n.specialty}`}
                            title="Eliminar"
                          >
      ...
      >
        <div className="p-6 sm:p-8">
          <h3 className="mb-4 text-lg font-semibold text-gray-900 dark:text-white">
            {editing ? "Editar nomenclador" : "Nuevo nomenclador"}
          </h3>
          ...
          <div className="space-y-4">
            <div>
              <Label htmlFor="nomen-discipline">Disciplina</Label>
              <Input
                id="nomen-discipline"
                placeholder="Ej. 5 - Ciencias Sociales"
            ...
            <div>
              <Label htmlFor="nomen-subdiscipline">Subdisciplina</Label>
              <Input
                id="nomen-subdiscipline"
                placeholder="Ej. 41 - Derecho y Jurisprudencia"
                value={form.subdiscipline}
                onChange={(e) =>
                  setForm({ ...form, subdiscipline: e.target.value })
                }
              />
            </div>
            <div>
              <Label htmlFor="nomen-specialty">Especialidad</Label>
              <Input
                id="nomen-specialty"
                placeholder="Ej. 99 - Otros (Derecho Público)"
      ...
      >
        <div className="p-6 sm:p-8">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            ¿Estás seguro de eliminar el nomenclador{" "}
            <span className="font-medium text-gray-700 dark:text-gray-300">
              {deleteTarget
                ? `${deleteTarget.discipline} / ${deleteTarget.subdiscipline} / ${deleteTarget.specialty}`
                : ""}
            </span>
